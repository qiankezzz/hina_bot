...
def string_to_image(text: str, font_path: str, font_size: int, image_path: str) -> None:
    ...
    for word in words[1:]:
        if font.getsize(current_line + ' ' + word)[0] <= 64 * font_size:
            current_line += ' ' + word
        else:
    ...
    image_width = max(text_widths)
    image_height = sum(text_heights)
