        ...
        # 直接发送
            ...
            for i, msg in enumerate(msg_list):
                ...
                try:
                    ...
                    await matcher.send(Message("结果返回字数过多，将以图片形式发送!"))
                    path = "./data/BingChat/temp_text_image.png"
                    string_to_image(str(msg),'arial.ttf',24,path)
                    path =  "file:///" + os.path.abspath(path)
                    data = await matcher.send(message=MessageSegment.image(path))
                    plugin_data.reply_message_id_dict[data['message_id']] = UserInfo(
                        platform='qq', user_id=event.user_id
                    )
