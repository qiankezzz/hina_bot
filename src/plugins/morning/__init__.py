...
morning_setting = on_regex(pattern=r"^早安(开启|关闭|设置)( (时限|多重起床|超级亢奋)(( \d{1,2}){1,2})?)?$",
...
async def good_morning(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    async def get_msg(**kwargs):
        return morning_manager.get_morning_msg(str(gid), str(uid), sex_str)

    await matcher.send(message=await get_msg(user_id=uid, group_id=gid, bot=bot), at_sender=True)
...
async def good_night(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    @add_coin_after_call()
    async def get_msg(**kwargs):
        return morning_manager.get_night_msg(str(gid), str(uid), sex_str)
