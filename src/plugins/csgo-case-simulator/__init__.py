...
async def handle_open_crate(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    @check_coins_required(amount * 2)
    async def open_crate(**kwargs):
        if name:
            ...
            if crate:
                ...
                await bot.delete_msg(message_id=opening_message_id['message_id'])
                try:
                    image = await utils.merge_images(opened_skins)
                except Exception as e:
                    nonebot.logger.error("合成图片失败" + e)
                    await crate_opening.finish("请求出现错误,请重试!")
                await crate_opening.send(MessageSegment.reply(event.message_id)+MessageSegment.image(image))
            else:
                await crate_opening.finish("箱子不存在")
        else:
            await crate_opening.finish("请输入箱子名称")
    
    await open_crate(group_id=group_id, bot=bot, user_id=user_id)
